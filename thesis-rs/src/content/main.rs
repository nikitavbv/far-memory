...
pub fn main_content() -> Block {
    ...
    Block::Multiple(vec![
        // intro
        Block::Placeholder(Box::new(Block::SectionHeader("Вступ".to_owned())), "remove numbering".to_owned()),
        ...
        Block::Placeholder(
            Box::new(paragraph("Завданням цього курсового проекту було поставлено аналіз проблеми, її існуючих рішень, формалізація методу та розробка архітектури програмного забезпечення для надання програмно-визначеної віддаленої памʼяті у розподілених системах.")),
            "mentions coursework".to_owned(),
        ),
