...
У доступі до даних зазвичай є закономірності, які можна використовувати, а вимоги до швидкодії для такого програмного забезпечення не є жорсткими."),
        ...
        paragraph(vec![
          r#"Існуючі реалізації спираються на прості евристики: рахування кількості доступів до обʼєктів для їх групування, запит наступної сторінки для структури даних. Розвитком цього може бути використання більш складних моделей для керування групуванням обʼєктів,
переміщення сторінок у віддалену памʼять та з неї, вирішення проблеми фрагментації. Методи які слід розглянути: евристичні підходи, побудова FSM за зібраною статистикою,
...
обмежити негативний вплив на надійність програмного забезпечення, можна використовувати наступні методи для сторінок памʼяті у віддаленій памʼяті:"#
        ...
        Block::Image(ImageBlock::new("fault_tolerance.jpg".to_owned(), "розміщення частин данних на різних вузлах при використанні erasure coding".to_owned())),
        ...
        paragraph(
          "Для інтеграції у нове програмне забезпечення (те, де є можливість змінювати реалізацію) доцільним є використання розумних покажчиків (з існуючих реалізацій так робить Carbink та AIFM). В межах цієї роботи створюється бібліотека на мові програмування Rust,
        ...
        Block::Image(ImageBlock::new("integration.png".to_owned(), "використання розумного покажчика для розміщення даних у віддаленій памʼяті".to_owned())),
        paragraph(
...
багато обчислень (інших операцій) на кожен доступ до памʼяті."),
        ...
        paragraph("- евристичний підхід - наприклад, рахувати кількість доступів у деякому вікні часу для визначення гарячих та холодних блоків памʼяті.".to_owned()),
...
евристики для оптимізації розташування обʼєктів у памʼяті. Не має методів інтеграції без зміни коду програмного забезпечення.".into(),
        ]),
        paragraph(vec![
          bold("- hydra"),
