...
pub fn hydra() -> Block {
    Block::Multiple(vec![
        ...
        paragraph(vec![
            "Hydra інтегрується у програмне забезпечення через використання механізму підкачки у операційній системі ".into(),
            reference("Linux", Reference::for_website("The Linux Kernel", "kernel.org")),
            ". Компонент \
...
надає можливість інтегруватись у нове та існуюче програмне забезпечення без змін у код.".into(),
        ]),
        paragraph(vec![
            "Для забезпечення відмовостійкості, у цій роботі пропонується підхід під назвою CodingSets. Під час перенесення даних у памʼять \
...
доки відбувається відновлення даних на новий вузол.".into(),
        ]),
        paragraph("Низький рівень затримки доступу до даних у віддаленій памʼяті забезпечується використанням RDMA, в тому числі тим фактом що малі \
...
запису даних, ця реалізація віддаленої памʼяті відправляє дані парності асинхронно, очікуючи на успішний запис лише основної частини даних."),
        paragraph("Головним недоліком цієї реалізації віддаленої памʼяті є залежність від RDMA, що вимагає спеціального апаратного забезпечення (мережевих \
