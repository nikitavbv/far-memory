...
хвилину. Для встановлення значення цього рівня будується гістограмма за попередьо зібраною статистикою та обирається той рівень, який задовільняє вимогам по швидкодії."),
        paragraph("Виявлення сторінок які задовільняють критерію відбувається у фоновому режимі, після чого zswap виконує стиснення цих даних для звільнення памʼяті."),
        paragraph("Окремий компонент збирає статистику з усіх вузлів у кластері та за домогою алгоритмів машинного навчання оптимізує значення гіперпараметрів. Це дозволяє \
...
машинного навчання Gaussian Process (GP) Bandit, який є оптимізацією чорного ящику (black-box optimization). У роботі зазначається, що це підвищує ефективність системи \
на 30% у порівнянні з еврістичними підходами. Алгоритм генерує декілька можливих конфігурацій параметрів системи, які оцінюються за допомогою симуляції роботи за статистикою, \
що була зібрана протягом тижня. Найкраща конфігурація (за показником звільненної памʼяті) передається для використання на всьому кластері."),
