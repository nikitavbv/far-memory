...
хвилину. Для встановлення значення цього рівня будується гістограмма за попередьо зібраною статистикою та обирається той рівень, який задовільняє вимогам по швидкодії."),
        ...
        paragraph(vec![
            "Окремий компонент збирає статистику з усіх вузлів у кластері та за домогою алгоритмів машинного навчання оптимізує значення гіперпараметрів. Це дозволяє \
...
що була зібрана протягом тижня. Найкраща конфігурація (за показником звільненної памʼяті) передається для використання на всьому кластері.".into()
        ]),
