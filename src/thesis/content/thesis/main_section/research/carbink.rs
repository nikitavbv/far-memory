...
даних у віддаленій памʼяті через розумні покажчики. При розміщенні даних у віддаленій памʼяті вони \
...
переміщуються у першу чергу сторінки з найбільшою кількістю холодних обʼєктів.".into()
        ]),
        paragraph("Для забезпечення відмовостійкості, менеджер памʼяті постійно перевіряє стан інших вузлів (як тих, що \
зберігають дані, так і тих, на яких розміщене програмне забезпечення, в яке інтегрується віддалена памʼять) за допомогою \
healthcheck запитів. Якщо вузол обчислень (compute node) виходить з ладу, то вузлам що зберігають дані, надається команда \
на звільнення памʼяті. Якщо вузол зберігання даних (storage node) виходить з ладу, то дані відновляться з використанням \
частин парності (parity shards) які були розміщені на інших вузлах після кодування стиранням (кодування відбувається одразу \
на рівні декількох сторінок). На час відновлення даних та переміщення на новий вузол очікується більш високий рівень \
...
також його привʼязаність до програмної та апаратної інфраструктури, що використовується в компанії Google. Крім цього, ця реалізація \
