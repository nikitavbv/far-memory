...
pub fn carbink() -> Block {
    Block::Multiple(vec![
        ...
        paragraph("Carbink це система віддаленої памʼяті розроблена та протестована компанією Google в своїх центрах обробки даних. Ця реалізація фокусується на покращені відмовостійкості та зниженні рівня затримок при роботі з віддаленою памʼяттю."),
        paragraph("Система складається з декількох компонентів: memory nodes (вузли, що зберігають дані), compute nodes (програмне забезпечення, в яке інтегрується віддалена памʼять) та memory manager (менеджер памʼяті). Менеджер памʼяті керує розподілом фрагментів памʼяті по вузлах, що їх зберігають та перевіряє стан роботи цих вузлів. Важливим припущенням є те, що вважається що менеджер памʼяті завжди залишається в робочому стані. Мережевий звʼязок теж вважається стабільним."),
        paragraph("Для інтеграції у програмне забезпечення використовується бібліотека яка надає застосунку доступ до даних у віддаленій памʼяті через розумні показчики. При розміщенні даних у віддаленій памʼяті вони \
групуються у сторінки (spans). При цьому, використовується "),
