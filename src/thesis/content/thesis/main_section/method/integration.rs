...
у таких мовах як Rust чи ".into(),
            ...
            " імітують звичайні показчики, при цьому дають додаткові можливості: реалізація такого показчика може, наприклад, виконати додатковий код у момент, коли програма виконує доступ до \
нього.".into(),
        ]),
...
мов прогрумування що підтримують створення таких біндингів. Недоліком цього підходу є необхідність додаткових змін і менш зручний інтерфейс використання."),
        paragraph("При створенні розумного показчика (FarMemory<T>), він привʼязується до одного з проміжків памʼяті (span). Приклад використання розумного показчика для зберігання даних у віддаленій памʼяті показано \
...
програмним забезпеченням."),
        ...
        paragraph(vec![
            "Коли розумний показчик отримає запит на доступ до даних (у мові програмування Rust це реалізується операцією ".into(),
...
лічильник посилань зменшується. Клієнт віддаленої памʼяті переміщує проміжки памʼяті для зберігання у бекенд тільки коли лічильник посилань дорівнює нулю."),
        paragraph(vec![
            "При переміщенні обʼєкту за показчик у віддаленій памʼяті виникає проблема його перетворення на послідовність байт для представлення у памʼяті та передачі мережею чи запису на диск. Найбільш \
...
FarMemorySerialized<T>) який не копіює дані з памʼяті, а використовує механізм серіалізації. Для цього на тип T накладається вимога реалізувати ознаки (trait) ".into(),
            ...
            ", так как для більшості типів вона копіює дані з памʼяті без змін, уникаючи зайвих операцій. Іншими словами, цей підхід максимально близький до рекурсивного \
...
кількості блокувань основого потоку виконання шляхом зменьшення зайвих переміщень даних між локальною памʼяттю та памʼяттю віддалених вузлів."),
        paragraph(vec![
