...
має значення що відповідають очікуваному часу доступу для кожного проміжку, з яким працює клієнт віддаленої памʼяті. Параметри моделі зберігаються у локальній оперативній \
