...
до даних у памʼяті віддалених вузлів є значно більшим (десятки-сотні міллісекунд). Цей фактор швидкодії віддаленої памʼяті схематично показано на рисунку 2.4. \
...
час (для вибору проміжків використовується агоритм заміщення проміжків, що буде розглянуто більш детально далі). Аналогічно, це дозволяє уникнути блокування основного потоку для переміщення даних у локальну памʼять. Якщо припустити що фоновий потік працює максимально швидко та \
...
проміжків що використовується) безперервно передається з усіх вузлів обчилення на вузел керування. На вузлі керування, ця інформація зберігається та неперервно обробляється \
...
значиний час, тому адаптація параметрів відбувається у фоновому потоці та не блокує роботу ні вузла керування, ні вузлів обчислень. Як тільки процес підбору параметрів \
завершується, то нові параметри моделі прогнозування надсилаються вузлам обчислення для використанні у алгоритмі заміщення проміжків. Після цього, вузел керування одразу \
...
має значення що відповідають очікваному часу доступу для кожного проміжку, з яким працює клієнт віддаленої памʼяті. Параметри моделі зберігаютсья у локальній оперативній \
...
для завчасного звільнення памʼяті або переміщення проміжків у локальну памʼять)."),
        ...
        paragraph(vec![
            ...
            "). Цей алгоритм заміщення проміжків неперервно збирає статистику доступу до проміжків та надсилає її на вузел керування, який в свою чергу зберігає статистику з \
...
проміжків наступним чином: для переміщення проміжків у віддалену памʼять обираються ті проміжкі, що знаходяться у локальній памʼяті та зустрічаються у послідовності в останню \
...
алгоритм заміщення проміжків що базується на використанні рекурентної нейронної мережі (".into(),
            ...
            ") як моделі прогнозвання доступу до проміжків. Рекурентні нейронні мережі є розповсюдженим рішенням у задачах де необхідно обробляти послідовності даних. \
...
цей алгоритм неперевно збирає та надсилає на вузел керування статистику доступів до проміжків. Вузел керування у фоновому потоці адаптації параметрів, агрегує статистику \
...
Для завчасного переміщення у локальну памʼять навпаки обираються проміжкі з найвижчим значенням.".into(),
        ]),
        paragraph(vec![
            "Слід зазначити, що наведені алгоритми заміщення проміжків є лише демонстрацією використання статистики доступу до памʼяті що неперевно збирається для \
