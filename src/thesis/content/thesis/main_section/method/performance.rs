...
памʼяті та використовується при кожній такій операції."),
        Block::Image(ImageBlock::new("./images/replacement_policy.jpg".to_owned(), "Використання алгоритму заміщення проміжків".to_owned())),
...
для завчасного звільнення памʼяті або переміщення проміжків у локальну памʼять)."),
        Block::Image(ImageBlock::new("./images/model.jpg".to_owned(), "Схематичне зображення принципу роботи моделі прогнозування доступу до проміжків".to_owned())),
        paragraph(vec![
