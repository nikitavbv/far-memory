...
з памʼяттю та характеристиках апаратного забезпечення (наприклад, швидкість мережі)."),
...
до даних у памʼяті віддалених вузлів є значно більшим (десятки-сотні мілісекунд). Цей фактор швидкодії віддаленої памʼяті схематично показано на рисунку 2.4. \
Коли програмне забезпечення запитує дані які знаходяться у віддаленій памʼяті, то доки \
...
часу доступу до даних, кількість а також довжину таких блокувань потрібно зменшувати."),
        Block::Image(ImageBlock::new("./images/latency.jpg".to_owned(), "Рівень часу доступу до даних в залежності від їх розміщення".to_owned()).with_scaling(0.75)),
        paragraph(vec![
...
забезпеченні для передачі даних часто використовуються протоколи віддаленого виклику процедур (remote procedure call), такі як ".into(),
...
протоколів не є оптимальним для клієнту віддаленої памʼяті, оскільки їх реалізації покладаються на складні алгоритми серіалізації запитів, що виконують зайві копіювання \
...
двосторонню (duplex) передачу. Наприклад, ".into(),
...
передавати та отримувати дані на швидкості 10 Гігабіт у секунду. У цій роботі, ця можливість апаратного забезпечення використовується клієнтом віддаленої памʼяті для \
...
повного переміщення невеликої кількості проміжків дозволяє знизити час доступу у гіршому випадку (tail latency) за рахунок більшої кількості операцій переміщення проміжків."),
...
проміжки з памʼяті віддалених вузлів у локальну памʼять. Крім цього, фоновий потік завчасно переміщує проміжки до яких з високою ймовірністю буде виконано доступ у найближчий \
час (для вибору проміжків використовується алгоритм заміщення проміжків, що буде розглянуто більш детально далі). Аналогічно, це дозволяє уникнути блокування основного потоку для переміщення даних у локальну памʼять. Якщо припустити що фоновий потік працює максимально швидко та \
...
проміжки, які у найближчий час виконання не будуть потрібні, то зменшується кількість блокувань для очікування переміщення потрібних проміжків. Як зазначалось раніше, \
...
завчасно переміщенні з більш повільної памʼяті у більш швидку. Вхідними даними є інформація про проміжки памʼяті до яких відбувався доступ під час поточного запуску програми. \
Кожен з проміжків ідентифікується номером, як зазначалось раніше. Надається інформація також про те, які проміжки памʼяті знаходяться у локальній памʼяті, \
...
переміщення з локальної памʼяті до віддалених вузлів і навпаки. Цей компонент може зберігати внутрішній стан між запитами."),
        paragraph("На рисунку 2.5 показано використання алгоритму заміщення проміжків для вибору проміжків для переміщення між локальною памʼяттю та памʼяттю віддалених \
вузлів (сам алгоритм схематично позначено символом f). Як можна побачити з рисунку, алгоритм заміщення проміжків є частиною процесу переміщення проміжків між типами \
памʼяті та використовується при кожній такій операції."),
        Block::Image(ImageBlock::new("./images/replacement_policy.jpg".to_owned(), "Використання алгоритму заміщення проміжків".to_owned()).with_scaling(0.5)),
...
розумного покажчика, за яким зберігаються дані у віддаленій памʼяті. Окремо також повідомляється про кожне переміщення проміжків між типами памʼяті. В залежності від \
реалізації (в цій роботі розглядається декілька), цей компонент може передавати статистику в агрегованому вигляді на вузол керування для збереження статистики \
...
є його неефективність для програмного забезпечення яке має інші закономірності роботи з памʼяттю."),
        paragraph("На рисунку 2.6 наведено приклад простої закономірності доступу до проміжків памʼяті у типового програмного забезпечення, де два проміжки (292 та 291) мають властивість \
декілька разів зустрічаються послідовно. Алгоритм заміщення проміжків міг би використовувати такі закономірності для більш ефективної роботи, наприклад в цьому випадку перемістивши \
проміжок з ідентифікатором 291 одразу після того, як програмне забезпечення запросило доступ до проміжку з ідентифікатором 291."),
        Block::Image(ImageBlock::new("./images/traces_example.png".to_owned(), "Приклад закономірностей доступу до пам'яті у програмного забезпеченні".to_owned())),
        paragraph("Оскільки у прикладному програмному забезпеченні закономірності роботи з памʼяттю можуть бути складними та залежати від багатьох факторів, то пошук \
алгоритму заміщення проміжків оптимального для задачі може бути складним для розробників, які використовують віддалену памʼять у своєму програмного забезпеченні. Це \
...
відбувається під час розіменування розумного покажчика на обʼєкт у віддаленій памʼяті (такий як FarMemoryLocal<T>) чи при роботі зі структурою даних адаптованої для роботи \
...
проміжків що використовується) безперервно передається з усіх вузлів обчислення на вузол керування. На вузлі керування, ця інформація зберігається та неперервно обробляється \
у фоновому потоці виконання. Зібрана статистика доступу до памʼяті з усіх вузлів використовується для адаптації параметрів моделі прогнозування. Цей процес може займати \
значний час, тому адаптація параметрів відбувається у фоновому потоці та не блокує роботу ні вузла керування, ні вузлів обчислень. Як тільки процес підбору параметрів \
завершується, то нові параметри моделі прогнозування надсилаються вузлам обчислення для використанні у алгоритмі заміщення проміжків. Після цього, вузол керування одразу \
починає нову ітерацію адаптації параметрів з використанням нової статистики і продовжує робити це у циклі весь час своєї роботи."),
        paragraph("Процес збору статистики та її використання для адаптації параметрів моделі прогнозування що використовується алгоритмом заміщення проміжків схематично \
зображено на рисунку 2.7."),
        Block::Image(ImageBlock::new("./images/span_replacement.jpg".to_owned(), "Збір статистики доступу до проміжків та її використання для адаптації параметрів алгоритму заміщення".to_owned()).with_scaling(0.8)),
        paragraph("Окремо слід розглянути випадок коли до алгоритму заміщення проміжків надходять запити на вибір проміжків для заміщення, але параметри моделі прогнозування \
ще не були отримані. Такий сценарій виникає на початку роботи віддаленої памʼяті. В такому випадку клієнт віддаленої памʼяті тимчасово використовує більш простий алгоритм \
заміщення проміжків обраний користувачем (наприклад, LRU). Статистика при цьому продовжується збиратись і як тільки будуть отримані параметри моделі прогнозування, то \
клієнт віддаленої памʼяті переключиться на використання алгоритму заміщення проміжків що базується на цій моделі."),
        paragraph("Прогнозна модель (зображено на рисунку 2.8) що використовується такими алгоритмами заміщення проміжків приймає на вхід історію доступів до проміжків памʼяті. На виході така модель \
має значення що відповідають очікуваному часу доступу для кожного проміжку, з яким працює клієнт віддаленої памʼяті. Параметри моделі зберігаютьроміся у локальній оперативній \
памʼяті вузла обчислення та є однаковими для усіх вузлів обчислення. Прогнозування з використанням моделі відбувається на кожен запит до алгоритму заміщення проміжків на \
вибір проміжків для переміщення (як зазначалось раніше, це відбувається в основному потоці виконання при необхідності звільнення памʼяті або в фоновому потоці виконання \
для завчасного звільнення памʼяті або переміщення проміжків у локальну памʼять)."),
        Block::Image(ImageBlock::new("./images/model.jpg".to_owned(), "Схематичне зображення принципу роботи моделі прогнозування доступу до проміжків".to_owned()).with_scaling(0.85)),
        paragraph(vec![
            "Одним з алгоритмів заміщення проміжків що розглядаються (та реалізовані) в цій роботі є алгоритм що базується на \"оптимальній моделі\" (використовує алгоритм ".into(),
            ...
            "). Цей алгоритм заміщення проміжків неперервно збирає статистику доступу до проміжків та надсилає її на вузол керування, який в свою чергу зберігає статистику з \
усіх вузлів обчислень та попередніх запусків програмного забезпечення, серед зібраних даних обирає найдовшу послідовність та надсилає її усім вузлам обчислення як параметр \
для алгоритму Беладі. Алгоритм заміщення проміжків обирає зміщення в послідовності що відповідає часу роботи програмного забезпечення, та обробляє запити на заміщення \
проміжків наступним чином: для переміщення проміжків у віддалену памʼять обираються ті проміжки, що знаходяться у локальній памʼяті та зустрічаються у послідовності в останню \
чергу у порівнянні з іншими проміжками відносно поточного зсуву. Аналогічно, для завчасного переміщення проміжків з віддаленої памʼяті у локальну обираються ті проміжки, які \
є найближчими у послідовності до поточного зсуву та знаходяться у памʼяті віддалених вузлів. Перевагою цього алгоритму заміщення проміжків є його простота у реалізації та \
використанні. Недоліком є те, що цей алгоритм є ефективним лише для програмного забезпечення що має постійні схеми доступу до памʼяті (наприклад, база даних що сканує увесь \
набір даних у однаковій послідовності кожного разу або нейронна мережа робота з вагами якої відбувається завжди однаковими чином).".into(),
        ]),
        paragraph(vec![
