...
зазвичай ключову роль у відмовостійкості програмного забезпечення грає деяка ймовірність що вузол обчислення вийде з ладу, то при використанні віддаленої памʼяті додається ймовірність що \
SSD диск (якщо він використовується як бекенд для зберігання даних) чи віддалений вузол (у випадку коли дані розміщуються на віддалених вузлах) вийде з ладу, що приведе до втрати частини даних. \
У випадку віддалених вузлів ризик збою стає значно високим, тому що з великою кількістю вузлів для втрати даних достатньо програмного чи апаратного збою на будь-якому з них, тобто ризик втрати даних \
...
Використання дискового простору можна зменшити використовуючи алгоритми стискання даних, такі як zstd, але це зробить систему більш повільною (найбільш швидкі алгоритми стискання даних обробляють \
...
відмовостійкості реалізовано за допомогою ReplicationBackend, який може бути за конфігурацією користувача використаний разом з NetworkNodeBackend."),
        paragraph(vec![
            "Найбільш ефективним методом забезпечення відмовостійкості є ".into(),
            reference("кодування стиранням (erasure coding)", Reference::for_website("Erasure Code // Wikipedia", "https://en.wikipedia.org/wiki/Erasure_code")),
            ". У цій роботі розглядається кодування за допомогою коду Ріда-Соломона, який є поширеним \
методом виправлення помилок у блоках даних. Суть цього методу полягає в тому, що дані поділяються на N частин та M додаткових, які обчислюються через лінійне перетворення. При втраті будь-яких M частин \
дані можна відновити через зворотнє лінійне перетворення. N та M задаються користувачем, приклади значень які можна взяти: (3, 2), (4, 3) та ін. - в залежності від середовища та вимог. Перевагою цього \
методу є швидке відновлення даних (так як час відновлення з памʼяті інших вузлів є нижчим за час відновлення з більш повільних пристроїв таких як SSD диски) та відносно невелике використання додаткової \
