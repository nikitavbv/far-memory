...
програма що зберігає дані у віддаленій памʼяті не має доступу до даних інших програм."),
        paragraph("Вузол обчислення складається з програмного забезпечення у яке інтегрована віддалена памʼять та клієнта віддаленої памʼяті. Програмне забезпечення працює з клієнтом віддаленої \
...
Саме переміщення даних у памʼять багатьох віддалених вузлів надає цій розробці практичної цінності."),
        paragraph(vec![
            "Тому вузли зберігання даних (storage nodes) є іншим компонентом системи. Як бекенд віддаленої памʼяті, цей вузол можна розглядати як сховище у форматі ключ-значення, де ключем \
...
Для цього, вузол керування відслідковує рівень завантаженості вузлів у кластері, та пріоритизує ті вузли, де найбільше вільної памʼяті. Крім цього, вузол керування відслідковує стан інших \
...
У випадку використання локальної памʼяті (як наприклад SSD диску) чи лише одного віддаленого вузла зберігання наявність вузла керування не є необхідним."),
        paragraph("Вузол керування також збирає статистику про роботу системи та визначає параметри її роботи (наприклад, умови за яких проміжки памʼяті переміщуються між локальною та \
...
керування параметрами роботи віддаленої памʼяті спрощує її використання для користувачів (розробників програмного забезпечення, у яке вона інтегрується) - це дозволяє уникнути проблеми \
...
роботи з памʼяттю програмного забезпечення як правило змінюються у часі і система повинна адаптувати параметри з урахуванням нової інформації."),
        ...
        paragraph(vec![
            "Кожен з цих компонентів надається користувачу як виконуваний файл для операційної системи Linux (за винятком клієнту віддаленої памʼяті який може інтегруватися у вузол \
обчислення за допомогою бібліотеки або окремого сервісу, що взаємодіє з механізмами керування памʼяттю операційної системи). Також надаються ".into(),
        ...
        paragraph(vec![
            "При використанні реалізації відаленної памʼяті на практиці виникає потреба у інтеграції з зовнішніми системами моніторингу для того, щоб одночасно контролювати як роботу \
...
віддаленої памʼяті і залишається незмінним увесь час роботи програми."),
        paragraph("Крім цього, вузол обчислення при встановленні зʼєднання з вузлом керування крім run id також передає таку інформацію як тип та версія програмного забезпечення. Це потрібно \
