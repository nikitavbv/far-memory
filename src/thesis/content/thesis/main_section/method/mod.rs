...
вузли зберігання та вузол керування. Визначено які дані та як передаються між цими вузлами, як виконується збір статистики про роботу кластеру та керування ним."),
        paragraph("З формулювання проблеми що вирішується, середовища та структури компонентів випливають підзадачі, які необхідно розглянути для створення програмного \
