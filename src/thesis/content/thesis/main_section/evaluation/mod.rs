...
pub fn evaluation() -> Block {
    Block::Multiple(vec![
        ...
        Block::UnorderedList(vec![
            "Яким є рівень швидкодії типового програмного забезпечення з різними схемами доступу до оперативної памʼяті при викори-станні віддаленої \
памʼяті?".into(),
...
таких як маршрутизатори чи комутатори. На цих серверах встановлена операційна система ".into(),
        ...
        paragraph("Віддалена памʼять була інтегрована у три синтетичні інформаційні системи, що були розроблені для цього дослідження."),
        paragraph(vec![
...
цю програму можна віднести до класу програмного забезпечення, що багато разів сканує весь робочий набір даних у незмінному порядку.".into()
        ]),
        paragraph(vec![
...
запиті обирається випадковий ідентифікатор, по якому отримується зображення з масиву зображень. Після цього, отримане зображення шифрується за допомогою ".into(),
            reference(
                "AES GCM",
                Reference::for_website("RFC 5288 - AES Galois Counter Mode (GCM) Cipher Suites for TLS", "https://datatracker.ietf.org/doc/html/rfc5288")
            ),
...
користувачів, а також відповідність зображень користувачам слідують ".into(),
            reference(
                "розподіленню Ципфа",
                Reference::for_publication("Zipf’s word frequency law in natural language: A critical review and future directions", "Steven T. Piantadosi", 2014, "Psychon Bull")
            ),
...
відповідають критеріям та рахується середня затримка прибуття літака у аеропорт призначення. Значення затримки повертається як відповідь на запит. \
...
з усіх проведених вимірювань."),
        paragraph("Результати вимірювання пропускної здатності наведено на графіку, що зображено на рисунку 4.1."),

        Block::Image(ImageBlock::new(
            "./images/plot_demo_throughput.png".to_owned(),
            "Пропускна здатність програмного забезпечення в залежності від його типу та обсягу локальної пам'яті".to_owned()
        ).with_scaling(0.5)),
...
веб сервісу з 80% локальної памʼяті та різним значенням параметру s розподілу Ципфа для розподілу ідентифікаторів користувачів у запитах та збережених \
ідентифікаторів зображень. При s = 0 розподіл є рівномірним, при s = 1 всі запити мають однакові ідентифікатори."),
        paragraph("Результати вимірювання пропускної здатності при різних значеннях параметру розподілу наведено на графіку, що зображено на рисунку 4.2."),
...
локальної памʼяті рівної 80% від усієї памʼяті що використовується для зберігання вагів нейронної мережі."),
        ...
        Block::UnorderedList(vec![
            "заміщення проміжків випадковим чином (random), тобто у разі необхідності переміщення проміжків з локальної пам'яті у памʼять віддалених вузлів проміжки \
...
віддалених вузлів обираються ті проміжки доступ до яких не виконувався найбільший інтервал часу. Цей алгоритм часто використовується для заміщення сторінок \
у операційних системах, у системах кешування та інших схожих задачах. Але програмне забезпечення що розглядається в цьому випадку є гарним прикладом \
програмного забезпечення з схемами доступу до памʼяті для яких використання LRU є неефективним. Оскільки доступ до вагів кожного шару нейронної мережі \
...
наведено на графіку, що зображено на рисунку 4.3."),
        ...
        paragraph("З графіку видно, що для цього типу програмного забезпечення алгоритм LRU є неефективним та призводить до низького рівня швидкодії при \
...
що працюють з цими даними."),
        paragraph("Приклад змін що було виконано наведено на рисунку 4.4 та рисунку 4.5."),

        Block::Image(ImageBlock::new(
            "./images/code_changes_data_structures.png".to_owned(),
            "Зміни у визначення структур даних для використання віддаленої пам'яті".to_owned()
        ).with_scaling(0.4)),

        Block::Image(ImageBlock::new(
            "./images/code_changes_signatures.png".to_owned(),
            "Зміни у визначення функцій для використання клієнту віддаленої пам'яті".to_owned(),
        ).with_scaling(0.4)),
...
локальної памʼяті. Було встановлено що запропонований метод надання віддаленої памʼяті є найбільш ефективним для програмного забезпечення що \
...
обʼєктами, має незмінні схеми доступу до памʼяті. Крім цього, віддалена памʼять є більш ефективною для програмного забезпечення що має зміщений \
...
проміжків що використовує статистику доступу до памʼяті є більш ефективним за прості евристики."),
